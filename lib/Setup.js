...
export default class Setup {
	...
	async run() {
		...
		let answerCourse = await inquirer.prompt(this.prompts.courseSource);

		await this.downloadFramework();
		await this.downloadCourse(answerCourse.courseSource);
	...
	downloadCourse(source) {
		const zip = endpoints[source];
		return download(zip, "builds/p101", { extract: true, strip: 1 });
	}
